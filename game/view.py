...
class GameView:
    ...
    def draw_enemies(self, enemies):
        for en in enemies.get():
            ...
            bar_height = 5
            pygame.draw.rect(
                self.win, RED, [en.rect.x, en.rect.y - 10, max_bar_width, bar_height])
            pygame.draw.rect(self.win, GREEN, [
                             en.rect.x, en.rect.y - 10, bar_width, bar_height])
    ...
    def draw_range(self, selected_tower):
        ...
        if selected_tower is not None:
            ...
            transparency = 120
            pygame.draw.circle(
                surface, (128, 128, 128, transparency), tw.rect.center, tw.range)
    ...
    def draw_hp(self, lives):
        ...
        for i in range(10):
            self.win.blit(HP_GRAY_IMAGE, (WIN_WIDTH // 2 -
                                          hp_rect.w * (2.5 - i % 5), hp_rect.h * (i // 5)))
        for i in range(lives):
            self.win.blit(HP_IMAGE, (WIN_WIDTH // 2 - hp_rect.w *
                                     (2.5 - i % 5), hp_rect.h * (i // 5)))
